...
1. `docs/output/` 收窄为 **report / prd / handoff** 三类；演示文稿移 `assets/decks/`（原 `output/ppt/` 删除）。
2. 新增 `docs/agents/handoff.md`——handoff 扩展为五种交接场景（项目内 / 跨 agent / 跨工具 / 跨项目 / 跨小组）。
...
- `git revert <本 commit>`；目录结构恢复需手动重建 `docs/output/ppt/`、回移 research 文件至 `prd/`。
