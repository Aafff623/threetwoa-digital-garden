...
> Baseline report: `docs/outputs/prd/nav-perf/latest-baseline-dev.md`
...
1. **Measurement seam**: Playwright soft-nav probe writing JSON/MD under `docs/outputs/prd/nav-perf/`. Metrics: `clickToPathMs`, `clickToContentMs`, `clickToQuietMs`, longtask count/max/total in a 2.5s window after click. Hard reload uses cache-disabled + reload (Ctrl+F5 analogue).
...
- [x] Re-measure after phases (`docs/outputs/prd/nav-perf/latest-after-p0p1-v2.md`)
