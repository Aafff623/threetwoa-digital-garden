...
#### 2  将下载好的 update.bat 和 update.py 移动到你本地博客项目的最外层根目录（也就是和 threetwoa-blog-manager、frontend 文件夹放在一起的地方）。
...
首先，进入 `threetwoa-blog-manager` 文件夹（**⚠️ 注意：请绝对不要重命名此文件夹，否则会导致环境路径解析失败！**）
...
**1. 配置本地物理路径**
打开控制台的“设置”页面。
在拉取的源码中，包含 `threetwoa-blog-manager` 和 `frontend` 两个核心文件夹。请在控制台中指定 `frontend` 的本地物理路径。
