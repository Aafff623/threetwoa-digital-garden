...
export default function WritingPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto w-full px-4 md:px-8 py-24 md:py-32">
      <div className="bg-cream/95 dark:bg-charcoal/95 backdrop-blur-md border border-charcoal/10 dark:border-white/12 rounded-[2.5rem] p-6 md:p-12 shadow-[0_12px_40px_-12px_rgba(0,0,0,0.12)]">
        {/* 头部标题与检索栏 */}
        <div className="border-b border-charcoal/8 dark:border-white/8 pb-8 mb-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          {/* 玻璃拟态搜索条 */}
          <div className="relative w-full lg:max-w-xs border border-charcoal/10 dark:border-white/12 bg-cream-dark/80 dark:bg-charcoal rounded-2xl px-4 py-2.5 flex items-center focus-within:border-gold/50 transition-colors shadow-sm">
          ...
          {paginatedPosts.length > 0 ? (
            paginatedPosts.map((post, idx) => {
              ...
              return (
                <div
                  key={post.slug}
                  className="writing-card group relative overflow-hidden rounded-[2.5rem] border border-charcoal/8 bg-cream dark:bg-charcoal shadow-sm backdrop-blur-sm transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_50px_-32px_rgba(217,134,95,0.22)] dark:border-white/12 flex flex-col md:flex-row items-stretch min-h-[420px] md:min-h-0 md:h-[250px]"
                >
                  {/* 封面图容器 - 绝对定位以支持无缝斜切分割 */}
